...
def train():
    with tf.Graph().as_default() as g:
        with tf.device('/gpu:'+str(GPU_INDEX)):
            INPUT_BATCH_SIZE = 32
            INTERP_BATCH_SIZE = 32
            pointclouds_pl, labels_pl = MODEL.placeholder_inputs(INPUT_BATCH_SIZE, NUM_POINT)
            ...
            with tf.control_dependencies(update_ops):
                init_op = optimizer.minimize(total_loss, global_step=batch, var_list = train_vars)
...
def train_one_epoch(sess, ops, train_writer, train_op):
    ...
    for batch_idx in range(num_batches):
        ...
        summary, step, _, loss_val= sess.run([ops['merged'], ops['step'],train_op, ops['total_loss']], feed_dict=feed_dict)
